...
layui.define(["layer", "laytpl", "form"], function (exports) {
  ...
  TreeTable.prototype.init = function () {
    ...
    var viewHtml =
      '<div class="layui-form ew-tree-table" style="' +
      (options.style || "") +
      '">';
    ...
    viewHtml += '            <table class="layui-table"></table>';
    viewHtml +=
      '            <div class="ew-tree-table-loading"><i class="layui-icon layui-anim layui-anim-rotate layui-anim-loop">&#xe63d;</i></div>';
    viewHtml +=
      '            <div class="ew-tree-table-empty" style="display: none;">' +
      (options.text.none || "") +
      "</div>";
    viewHtml += "         </div>";
    viewHtml += "      </div>";
    viewHtml +=
      '      <div class="ew-tree-table-border top"></div><div class="ew-tree-table-border left"></div>';
    viewHtml +=
      '      <div class="ew-tree-table-border right"></div><div class="ew-tree-table-border bottom"></div>';
    viewHtml += "   </div>";
    $elem.after(viewHtml);
    ...
    var headHtmlStr =
      colgroupHtmlStr + "<thead>" + options.getThead() + "</thead>";
    if (options.height) {
      // 固定表头
      ...
      if (options.height.indexOf("full-") == 0) {
        ...
        var cssStr =
          "<style>.ew-tree-table > .ew-tree-table-group > .ew-tree-table-box {";
    ...
    if (options.reqData) {
      ...
      if (options.data && options.data.length > 0) {
        ...
        if (options.tree.isPidData) {
          // pid形式数据
          options.data = treeTb.pidToChildren(
            options.data,
            options.tree.idName,
            options.tree.pidName,
  ...
  TreeTable.prototype.bindEvents = function () {
    ...
    $tbody.off("click.fold").on("click.fold", ".ew-tree-pack", function (e) {
      ...
      if ($tr.hasClass("ew-tree-table-loading")) {
        // 已是加载中
        return;
      }
      var haveChild = $tr.data("have-child");
      if (haveChild != true && haveChild != "true") {
        // 子节点
        return;
      }
      var id = $tr.data("id");
      ...
      if (
        !isOpen &&
        (!data[options.tree.childName] ||
          data[options.tree.childName].length <= 0)
      ) {
    ...
    // 绑定行双击事件
    $tbody
      .off("dblclick.rowDouble")
      .on("dblclick.rowDouble", "tr", function () {
        layui.event.call(
          this,
          MOD_NAME,
          "rowDouble(" + tbFilter + ")",
      ...
      if (edit) {
        ...
        if ("text" == edit || "number" == edit) {
          ...
          var $input = $(
            '<input type="' +
              edit +
              '" class="layui-input ew-tree-table-edit"/>',
          );
    ...
    // 绑定单元格双击事件
    $tbody
      .off("dblclick.cellDouble")
      .on("dblclick.cellDouble", "td", function (e) {
        var $td = $(this);
        var type = $td.data("type");
        // 判断是否是复选框、单选框列
        if (type == "checkbox" || type == "radio") {
  ...
  TreeTable.prototype.getComponents = function () {
    ...
    var $headTb = $group
      .children(".ew-tree-table-head")
      .children(".layui-table"); // 表头表格
  ...
  TreeTable.prototype.renderBody = function (data, indentSize, isHide) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (children && children.length > 0) {
        htmlStr += this.renderBody(
          children,
          indentSize + 1,
          !d[treeOption.openName],
        );
      }
    }
    return htmlStr;
  ...
  TreeTable.prototype.renderBodyTr = function (d, indentSize, isHide, $tr) {
    ...
    if ($tr) {
      ...
      htmlStr +=
        '<tr class="' + classNames + '" data-id="' + d[treeOption.idName] + '"';
      htmlStr +=
        ' data-pid="' +
        (d[treeOption.pidName] || "") +
        '" data-have-child="' +
        haveChild +
        '"';
  ...
  TreeTable.prototype.renderBodyTd = function (d, indentSize, index, $td) {
    ...
    if (col.type == "numbers") {
      ...
      attrStr += d.LAY_CHECKED ? ' checked="checked"' : "";
      fieldStr +=
        '<input type="radio" ' + attrStr + ' class="ew-tree-table-radio" />';
      col.singleLine = false;
    } else if (col.templet) {
      // 自定义模板
    ...
    if (index == treeOption.iconIndex) {
      ...
      var haveChild = getHaveChild(d, treeOption);
      tdStr +=
        '<i class="layui-icon ew-tree-table-arrow ' +
        (haveChild ? "" : "ew-tree-table-arrow-hide") +
        " " +
        (options.tree.arrowType || "") +
        '"></i>';
      // 加图标
    ...
    if (col.singleLine) {
      htmlStr +=
        '<div class="ew-tree-table-td-single"><i class="layui-icon layui-icon-close ew-tree-tips-c"></i><div class="ew-tree-tips">' +
        tdStr +
        "</div></div>";
    } else {
      htmlStr += tdStr;
  ...
  TreeTable.prototype.checkSubCB = function ($tr, checked) {
    ...
    $trList.each(function () {
      ...
      if (checked) {
        ...
        $cb.data("indeterminate", "false");
        $cb
          .next(".layui-form-checkbox")
          .removeClass("layui-form-checked ew-form-indeterminate");
      }
  ...
  TreeTable.prototype.checkParentCB = function ($tr) {
    ...
    if (ckNum > 0 && unCkNum == 0) {
      ...
      $cb
        .next(".layui-form-checkbox")
        .addClass("layui-form-checked ew-form-indeterminate");
      that.update($tr.data("id"), {LAY_CHECKED: true}); // 同步更新数据
    }
  };
  ...
  TreeTable.prototype.checkStatus = function (needIndeterminate) {
    ...
    if ($radio.length > 0) {
      ...
      // 获取复选框数据
      $table
        .find('input[name="' + checkboxFilter + '"]:checked')
        .each(function () {
          var id = $(this).val();
          var isIndeterminate = $(this)
            .next(".layui-form-checkbox")
            .hasClass("ew-form-indeterminate");
  ...
  function getThead(options) {
    ...
    for (var i = 0; i < options.cols.length; i++) {
      ...
      col.align && (htmlStr += ' align="' + col.align + '"'); // 对齐方式
      htmlStr += " >";
      if (col.singleLine && col.type != "checkbox") {
        // 单行显示
        htmlStr +=
          '<div class="ew-tree-table-td-single"><i class="layui-icon layui-icon-close ew-tree-tips-c"></i><div class="ew-tree-tips">';
      }
  ...
  function getTbWidth(options) {
    ...
    for (var i = 0; i < options.cols.length; i++) {
      ...
      if (col.type == "space") {
        ...
        // 复/单选框列
        minWidth += 48;
      } else if (!col.width || /\d+%$/.test(col.width)) {
        // 列未固定宽度
        setWidth = false;
        if (col.minWidth) {
  ...
  $(window).resize(function () {
    $(".ew-tree-table").each(function () {
      ...
      var $tbBox = $(this)
        .children(".ew-tree-table-group")
        .children(".ew-tree-table-box");
  ...
  // 表格溢出点击展开功能
  $(document)
    .on("mouseenter", ".ew-tree-table td", function () {
      var $tdSingle = $(this).children(".ew-tree-table-td-single");
      var $content = $tdSingle.children(".ew-tree-tips");
      if (
        $tdSingle.length > 0 &&
        $content.prop("scrollWidth") > $content.outerWidth()
      ) {
        $(this).append(
          '<div class="layui-table-grid-down"><i class="layui-icon layui-icon-down"></i></div>',
        );
      }
    })
  ...
  $(document).on(
    ...
    function (e) {
      ...
      if (
        $tdSingle.outerWidth() + $tdSingle.parent().offset().left >
        $box.offset().left + $box.outerWidth()
      ) {
        $tdSingle.addClass("ew-show-left");
      }
      if (
        $tdSingle.outerHeight() + $tdSingle.parent().offset().top >
        $box.offset().top + $box.outerHeight()
      ) {
  ...
  function addPidField(data, treeOption, parent) {
    for (var i = 0; i < data.length; i++) {
      ...
      if (
        data[i][treeOption.childName] &&
        data[i][treeOption.childName].length > 0
      ) {
    ...
    pidToChildren: function (data, idName, pidName, childName, pId) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (pidEquals(data[i][pidName], pId)) {
          var children = this.pidToChildren(
            data,
            idName,
            pidName,
            childName,
