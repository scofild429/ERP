  ...
  layuimini = new (function () {
    ...
    this.init = function (url) {
      ...
      $.getJSON(url, function (data, status) {
        //        console.log(data); // that the the init.json file
        if (data == null) {
    ...
    this.initDevice = function () {
      if (layuimini.checkMobile()) {
        ...
        $(".layui-layout-body").attr(
          "class",
          "layui-layout-body layuimini-mini",
        );
      }
    };
    ...
    this.initHome = function () {
      sessionStorage.setItem(
        "layuiminiHomeHref",
        "resources/page/welcome-1.html",
      );
      $("#layuiminiHomeTabId").html(
        '<i class="fa fa-home"></i> <span>工作台</span>',
      );
      $("#layuiminiHomeTabId").attr("lay-id", "resources/page/welcome-1.html");
      $("#layuiminiHomeTabIframe").html(
        '<iframe width="100%" height="100%" frameborder="0"  src="resources/page/welcome-1.html"></iframe>',
      );
    };
            ...
            var buildChildHtml = function (html, child, menuParameId) {
              ...
              $.each(child, function (childIndex, childMenu) {
                ...
                if (childMenu.child != undefined && childMenu.child != []) {
                  ...
                  html +=
                    '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-tab-mpi="m-p-i-' +
                    menuParameId +
                    '" data-tab="' +
                    childMenu.href +
                    '" target="' +
    ...
    this.initTab = function () {
      ...
      if (urlArr.length >= 2) {
        ...
        if (!checkTab) {
            ...
            if (checkHref == tabId) {
              ...
              var addMenuClass = function ($element, type) {
                if (type == 1) {
                  ...
                  if (
                    $element.attr("class") != "layui-nav-item layui-nav-itemed"
                  ) {
                    addMenuClass($element.parent().parent(), 2);
                  } else {
                    var moduleId = $element.parent().attr("id");
          ...
          // 非菜单打开的tab窗口
          if (href == title) {
            var layuiminiTabInfo = JSON.parse(
              sessionStorage.getItem("layuiminiTabInfo"),
            );
    ...
    this.addTab = function (tabId, href, title, addSession) {
      if (addSession == undefined || addSession == true) {
        ...
        sessionStorage.setItem(
          "layuiminiTabInfo",
          JSON.stringify(layuiminiTabInfo),
        );
      }
      element.tabAdd("layuiminiTab", {
        title:
          title +
          '<i data-tab-close="" class="layui-icon layui-unselect layui-tab-close">ဆ</i>', //用于演示
        content:
      ...
      $(window)
        .on("resize", function (event) {
          ...
          if (topTabsWidth > topTabsBoxWidth) {
            if (
              tabLi.position().left > topTabsBoxWidth ||
